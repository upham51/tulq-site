...
```
tulq-site/                        ← tulq.health (tribal / IHS track)
├── index.html                    ← landing page          }
├── story.html                    ← Ollie Moses + Chemawa } hand-written,
├── contact.html                  ←                       } edit directly
├── privacy.html                  ← noindex (see below)   }
├── for/                          ← pillar pages          ┐
│   ├── tribal-health-ihs.html    │
│   └── contracting-officers.html │
├── buy-indian-act.html           │
├── areas/                        ← 12 IHS Area pages + index
├── resources/                    ← 6 explainers + index  │ GENERATED —
├── compare/                      ← tribal comparison     │ edit the
├── styles.css                    ← hand-written pages    │ tools/content_*.py
├── pages.css                     ← generated pages       │ modules, then
├── assets/                       ← self-hosted imagery   │ rerun the builder
├── sitemap.xml                   ← regenerated by builder ┘
├── care/                         ← tulqhealth.com (mainstream track),
│                                   a self-contained mirror with its own
│                                   assets, CSS, sitemap and _headers
└── tools/
    ├── build-pages.py            ← generates every content page + sitemaps
    ├── pagekit.py                ← shared shell, nav, footer, schema
    ├── content_*.py              ← the actual page copy
    ├── check-links.py            ← verifies every internal link resolves
    ├── fetch-optimize-images.py  ← pulls remote imagery into assets/
    └── rewrite-image-refs.py     ← points HTML at the local copies
```

### Regenerating content pages

```bash
python3 tools/build-pages.py    # writes 37 pages + both sitemaps
python3 tools/check-links.py    # verifies nothing 404s
```

There is still **no build step at deploy time** — Cloudflare Pages serves the
committed HTML directly. The generator exists so three dozen pages share one
nav, footer, and schema shape. Edit `tools/content_*.py`, not the generated
HTML; hand edits are overwritten on the next run.

Both `privacy.html` files are `noindex, follow` and excluded from the sitemaps:
they are near-identical across the two domains, and one indexed copy is better
than a cross-domain duplicate. See `CLAUDE.md` for the full SEO conventions.
